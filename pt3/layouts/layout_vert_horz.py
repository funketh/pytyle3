...
from pt3.layouts import store


class OrientLayout(Layout, metaclass=ABCMeta):
    ...
    def _get_focused(self):
        ...
        if type(state.active_window) == list:
            state.active_window = state.active_window[0]

        if state.active_window not in client.clients:
            return None

        awin = client.clients[state.active_window]
